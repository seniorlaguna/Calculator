...
class CalculationAdapter(private val mainActivity: MainActivity) : RecyclerView.Adapter<CalculationAdapter.CalculationViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CalculationViewHolder {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.scientific_calculator_history_item, parent, false)

        view.setOnClickListener(mainActivity.scientificFragment.historyFragment)
        view.setOnLongClickListener(mainActivity.scientificFragment.historyFragment)
